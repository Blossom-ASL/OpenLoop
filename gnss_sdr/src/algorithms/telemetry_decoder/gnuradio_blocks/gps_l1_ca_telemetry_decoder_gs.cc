                            ...
                            d_dump_crc_stats(conf.dump_crc_stats)
{
    ...
    //tow to tracking
    this->message_port_register_out(pmt::mp("tow_to_trk"));
            ...
            if (subframe_ID > 0 && subframe_ID < 6)
                {
                    ...
                    std::cout << "New GPS NAV message received in channel " << this->d_channel << ": "
                              ...
                              << " with Doppler=" << std::setprecision(2) << cn0 << std::setprecision(default_precision)
                              << " Hz" << std::endl;

                    
                    //send_tow_to_tracking(static_cast<double>(d_TOW_at_current_symbol_ms) / 1000.0);
                    return true;
                }
        }
    return false;
...
void gps_l1_ca_telemetry_decoder_gs::send_tow_to_tracking(double tow_seconds)
{
    gr::thread::scoped_lock lock(d_setlock);
    
    try
        {
            // Create a structure or use a simple double for TOW
            //std::cout << "Reached in send_tow_to_tracking" << std::endl;
            this->message_port_pub(pmt::mp("tow_to_trk"), pmt::make_any(tow_seconds));

            DLOG(INFO) << "Sent TOW " << tow_seconds << " s to tracking channel " 
                       << d_channel;
        }
    catch (const std::exception &e)
        {
            LOG(WARNING) << "Failed to send TOW to tracking: " << e.what();
        }
}
                ...
                if (abs(corr_value) >= d_samples_per_preamble)
                    {
                        ...
                        d_prev_GPS_frame_4bytes = 0;
                        if (decode_subframe(current_gs.Carrier_Doppler_hz, d_flag_PLL_180_deg_phase_locked))
                            {
                                d_CRC_error_counter = 0;
