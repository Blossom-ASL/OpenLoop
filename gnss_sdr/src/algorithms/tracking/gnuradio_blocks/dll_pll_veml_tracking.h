...
private:
    ...
    void msg_handler_tow_from_telemetry(const pmt::pmt_t &msg);
    void do_correlation_step(const gr_complex *input_samples);
    void do_correlation_step_ED(const gr_complex *input_samples);
    void run_dll_pll();
    void check_carrier_phase_coherent_initialization();
    void update_tracking_vars();
    void update_tracking_vars_ED();
    ...
    // Returns number of leap seconds (update when needed)
    int get_leap_seconds(time_t gps_time);

    

// Convert GPS week + TOW to UTC time
    std::tm gpsTowToUTC(int gps_week, double tow_sec);

    void init_doppler_calculator(const std::string& sp3_file_path,
                                 double receiver_lat,
                                 double receiver_lon, 
                                 double receiver_alt);
    
        // Get predicted doppler
    double get_predicted_doppler();
    
    // Print doppler information
    void print_doppler_info();
    ...
    double d_code_freq_chips_ED;
    double d_carrier_doppler_hz;
    double d_carrier_doppler_hz_ED;
    double d_acc_carrier_phase_rad;
    double d_rem_code_phase_chips;
    double d_rem_code_phase_chips_ED;
    double d_T_chip_seconds;
    double d_T_prn_seconds;
    double d_T_prn_samples;
    double d_K_blk_samples;
    double d_T_chip_seconds_ED;
    double d_T_prn_seconds_ED;
    double d_T_prn_samples_ED;
    double d_K_blk_samples_ED;
    double d_carrier_lock_test;
    double d_CN0_SNV_dB_Hz;
    double d_carrier_lock_threshold;
    double d_carrier_phase_step_rad;
    double d_carrier_phase_step_rad_ED;

    double d_carrier_phase_rate_step_rad;
    double d_code_phase_step_chips;
    double d_code_phase_step_chips_ED;

    double d_code_phase_rate_step_chips;
    double d_rem_code_phase_samples;
    double d_rem_code_phase_samples_ED;

    double d_TOW_at_current_symbol_s;
    ...
    std::unique_ptr<SatelliteDopplerCalculator> doppler_calculator;
    ...
    int32_t d_current_prn_length_samples_ED;
    ...
    //Time data for doppler correction
    int32_t d_current_gps_week_;
    int32_t d_satellite_PRN;
    double d_current_gps_time_;  // GPS seconds of week
    double d_sample_counter;   // Total samples processed
    double d_fs_in; 
    ...
    bool doppler_calculator_initialized;
