...
#define GPS_UNIX_OFFSET 315964800  // 1970 → 1980 offset
...
dll_pll_veml_tracking::dll_pll_veml_tracking(const Dll_Pll_Conf &conf_)
    : gr::block("dll_pll_veml_tracking", gr::io_signature::make(1, 1, sizeof(gr_complex)),
      ...
      d_carrier_doppler_hz_ED(0.0),
      d_acc_carrier_phase_rad(0.0),
      d_rem_code_phase_chips(0.0),
      d_rem_code_phase_chips_ED(0.0),
      d_T_chip_seconds(0.0),
      d_T_prn_seconds(0.0),
      d_T_prn_samples(0.0),
      d_K_blk_samples(0.0),
      d_T_chip_seconds_ED(0.0),
      d_T_prn_seconds_ED(0.0),
      d_T_prn_samples_ED(0.0),
      d_K_blk_samples_ED(0.0),
      d_carrier_lock_test(1.0),
      d_CN0_SNV_dB_Hz(0.0),
      d_carrier_lock_threshold(d_trk_parameters.carrier_lock_th),
      d_carrier_phase_step_rad(0.0),
      d_carrier_phase_step_rad_ED(0.0),
      d_carrier_phase_rate_step_rad(0.0),
      d_code_phase_step_chips(0.0),
      d_code_phase_step_chips_ED(0.0),
      d_code_phase_rate_step_chips(0.0),
      d_rem_code_phase_samples(0.0),  // Residual code phase (in chips)
      d_rem_code_phase_samples_ED(0.0),  // Residual code phase (in chips)
      ...
      d_current_prn_length_samples_ED(static_cast<int32_t>(d_trk_parameters.vector_length)),
      ...
      doppler_calculator(nullptr),
      doppler_calculator_initialized(false)
{
    ...
    this->message_port_register_in(pmt::mp("tow_to_trk"));
    this->set_msg_handler(
        pmt::mp("tow_to_trk"),
#if HAS_GENERIC_LAMBDA
        [this](auto &&PH1) { msg_handler_tow_from_telemetry(PH1); });
#else
#if USE_BOOST_BIND_PLACEHOLDERS
        boost::bind(&dll_pll_veml_tracking::msg_handler_tow_from_telemetry, this, boost::placeholders::_1));
#else
        boost::bind(&dll_pll_veml_tracking::msg_handler_tow_from_telemetry, this, _1));
#endif
#endif
    try {
    init_doppler_calculator(
        ...
        std::cerr << "Warning: Could not initialize doppler calculator: " 
                    << e.what() << std::endl;
    }


    // std::cout<<"Carrier Lock Threshold"<<d_carrier_lock_threshold<<std::endl;
...
void dll_pll_veml_tracking::init_doppler_calculator(
    const std::string& sp3_file_path,
    double receiver_lat,
    double receiver_lon,
    double receiver_alt)
{
    std::cout << "Initializing Doppler Calculator..." << std::endl;
    
    try {
        // Create calculator instance
        doppler_calculator = std::make_unique<SatelliteDopplerCalculator>();
        
        // Set receiver position
        doppler_calculator->setReceiverPosition(receiver_lat, receiver_lon, receiver_alt);
        
        // Set SP3 file path
        doppler_calculator->setSP3FilePath(sp3_file_path);
        
        // Load SP3 file
        if (doppler_calculator->loadSP3File()) {
            doppler_calculator_initialized = true;
            std::cout << "✓ Doppler calculator initialized successfully" << std::endl;
        } else {
            std::cerr << "✗ Failed to load SP3 file" << std::endl;
            doppler_calculator_initialized = false;
        }
    } catch (const std::exception& e) {
        std::cerr << "Error initializing doppler calculator: " << e.what() << std::endl;
        doppler_calculator_initialized = false;
    }
}


double dll_pll_veml_tracking::get_predicted_doppler() {
    if (!doppler_calculator_initialized || !doppler_calculator) {
        std::cerr << "Doppler calculator not initialized" << std::endl;
        return 0.0;
    }
    ...
    if (utc_time.tm_year < 0) {
        //std::cerr << "Invalid UTC time" << std::endl;
        return 0.0;
    }
    
    // Format satellite ID (e.g., PRN 1 -> "G01")
    //std::cout << "Reached Calling get doppler" <<std::endl;
    char sat_id[4];
    std::snprintf(sat_id, sizeof(sat_id), "G%02d", d_satellite_PRN);
    
    // Get doppler shift
    double doppler = doppler_calculator->getDopplerShift(std::string(sat_id), utc_time);
...
void dll_pll_veml_tracking::print_doppler_info() {
    if (!doppler_calculator_initialized || !doppler_calculator) {
        std::cerr << "Doppler calculator not initialized" << std::endl;
        return;
    }
    ...
    doppler_calculator->printDopplerInfo(std::string(sat_id), utc_time);
}
...
void dll_pll_veml_tracking::msg_handler_tow_from_telemetry(const pmt::pmt_t &msg)
{
    try
        {
            double tow_seconds = wht::any_cast<double>(pmt::any_ref(msg));
            
            gr::thread::scoped_lock lock(d_setlock);
            
            d_TOW_at_current_symbol_s = tow_seconds;
            d_TOW_valid = true;
            
            // std::cout << "Reached in send_tow_to_tracking: " << d_TOW_at_current_symbol_s<<std::endl;
            
            // DLOG(INFO) << "Received TOW: " << tow_seconds 
            //            << " s in channel " << this->d_channel;
        }
    catch (const wht::bad_any_cast &e)
        {
            LOG(WARNING) << "msg_handler_tow Bad any_cast: " << e.what();
        }
}
...
void dll_pll_veml_tracking::start_tracking()
{
    ...
    d_satellite_PRN = d_acquisition_gnss_synchro->PRN;
...
void dll_pll_veml_tracking::do_correlation_step_ED(const gr_complex *input_samples)
{
    // ################# CARRIER WIPEOFF AND CORRELATORS ##############################
    // perform carrier wipe-off and compute Early, Prompt and Late correlation
    ...
    d_multicorrelator_cpu.Carrier_wipeoff_multicorrelator_resampler(
        d_rem_carr_phase_rad,
        static_cast<float>(d_carrier_phase_step_rad_ED), static_cast<float>(d_carrier_phase_rate_step_rad),
        static_cast<float>(d_rem_code_phase_chips_ED) * static_cast<float>(d_code_samples_per_chip),
        static_cast<float>(d_code_phase_step_chips_ED) * static_cast<float>(d_code_samples_per_chip),
        static_cast<float>(d_code_phase_rate_step_chips) * static_cast<float>(d_code_samples_per_chip),
        d_trk_parameters.vector_length);
...
//     if(d_TOW_valid)
//     {
//         //s::cout << "Reached inside checking d_TOW_Valid" <<std::endl;
//         std::tm utc_time = gpsTowToUTC(2321, d_TOW_at_current_symbol_s);
//         // std::cout << "Year: " << utc_time.tm_year + 1900 << std::endl;
// // // Debug print
// if(utc_time.tm_year>-1)
// {
// std::cout << "UTC time = "
...
// // std::cout << "UTC time = " << std::put_time(utc_time, "%Y-%m-%d %H:%M:%S") << std::endl;
// }
   // }
    if (d_cloop)
        {
    ...
    // std::cout<<"d_carr_phase_error_hz is: "<<d_carr_phase_error_hz<<std::endl; 
    ...
    // float random_offset = 6.0*dist(rng);
    // std::cout<<"Random Offset is: "<<random_offset <<std::endl;
...
void dll_pll_veml_tracking::update_tracking_vars()
{
    ...
    if (d_trk_parameters.high_dyn)
        {
            std::cout <<"d_trk_parameters.high_dyn: "<<d_trk_parameters.high_dyn<<std::endl;
...
// Update parameters for correlation based on external doppler
void dll_pll_veml_tracking::update_tracking_vars_ED()
{
    d_T_chip_seconds_ED = 1.0 / d_code_freq_chips_ED;
    d_T_prn_seconds_ED = d_T_chip_seconds_ED * static_cast<double>(d_code_length_chips);

    // // ################## CARRIER AND CODE NCO BUFFER ALIGNMENT #######################
    // // keep alignment parameters for the next input buffer
    // // Compute the next buffer length based in the new period of the PRN sequence and the code phase error estimation
    d_T_prn_samples_ED = d_T_prn_seconds_ED * d_trk_parameters.fs_in;
    d_K_blk_samples_ED = d_T_prn_samples_ED + d_rem_code_phase_samples_ED;
    d_current_prn_length_samples_ED = static_cast<int32_t>(std::floor(d_K_blk_samples_ED));  // round to a discrete number of samples

    // // ################### PLL COMMANDS #################################################
    // // carrier phase step (NCO phase increment per sample) [rads/sample]
    d_carrier_phase_step_rad_ED = TWO_PI * d_carrier_doppler_hz_ED / d_trk_parameters.fs_in;
    // // carrier phase rate step (NCO phase increment rate per sample) [rads/sample^2]
    ...
    // // ################### DLL COMMANDS #################################################
    // // code phase step (Code resampler phase increment per sample) [chips/sample]
    d_code_phase_step_chips_ED = d_code_freq_chips_ED / d_trk_parameters.fs_in;
    
    // // remnant code phase [chips]
    d_rem_code_phase_samples_ED = d_K_blk_samples_ED - static_cast<double>(d_current_prn_length_samples_ED);  // rounding error < 1 sample
    d_rem_code_phase_chips_ED = d_code_freq_chips_ED * d_rem_code_phase_samples_ED / d_trk_parameters.fs_in;
}
...
int dll_pll_veml_tracking::get_leap_seconds(time_t gps_time)
{
    return 18;
}


// Main function to call inside tracking
std::tm dll_pll_veml_tracking::gpsTowToUTC(int gps_week, double tow_sec)
{
    // GPS seconds since Unix epoch
    time_t gps_time =
        gps_week * SECONDS_IN_WEEK +
        (time_t)tow_sec +
        GPS_UNIX_OFFSET;

    // Apply leap second correction
    int leap = get_leap_seconds(gps_time);
    gps_time -= leap;
    ...
    if (utc_tm != nullptr) {
        ...
        result.tm_year = -1;  // Mark as invalid
    }
    return result;
}
        ...
        case 1:  // Pull-in
            {
                ...
                d_code_freq_chips = d_code_chip_rate;
                d_code_freq_chips_ED = d_code_chip_rate;
                d_code_phase_step_chips = d_code_freq_chips / d_trk_parameters.fs_in;
                d_code_phase_rate_step_chips = 0.0;
